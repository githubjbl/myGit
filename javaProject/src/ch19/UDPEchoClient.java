...
public class UDPEchoClient {
	...
	public UDPEchoClient(String ip, int port) {
		try {
			...
			str = file.readLine();
			byte buffer[] = str.getBytes(); //스트링을 바이트배열로 변환(UDP통신에서는 byte배열로 송수신
			DatagramPacket dp = new DatagramPacket(buffer, buffer.length, ia, SERVERPORT);
			ds.send(dp);
			buffer = new byte[512]; //서버에서 보낸 메시지를 저장하기 위한 바이트 배열 처리
	...
	public static void main(String[] args) {
		new UDPEchoClient("localhost", 2000);//(클라이언트ip,클라이언트 포트번호)
		//UDP는 클라이언트 포트번호와 서버측ㄷ 포트번호가 달라야한다.
